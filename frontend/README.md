...
│   ├── compliance.ts     # Compliance configuration and checks
│   ├── contracts.ts      # Smart contract info and KYC verification
│   └── identity.ts       # On-chain identity management
...
- **`compliance.ts`** - `complianceApi` for compliance configuration and checks
- **`contracts.ts`** - `contractsApi` for contract info and KYC verification
- **`identity.ts`** - `identityApi` for on-chain identity registration and claims
